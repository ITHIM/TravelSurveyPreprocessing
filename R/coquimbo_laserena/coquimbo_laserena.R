#' ---
#' title: "Preprocessing of Coquimbo and La Serena's travel dataset"
#' author: "Daniel"
#' output:
#'   html_document:
#'     toc: true
...
#' ## Documentation
#' These files are available in the v-drive in the path "V:/Studies/MOVED/HealthImpact/Data/Country/Chile/Travel Surveys/Coquimbo_LaSerena/". Locally, this documentation is located in ".../Chile/Coquimbo_LaSerena/Trips/Reports".
...
data.frame(
  ...
  Description = c("Technical report and final results",
                  ""),
  Title = c("Actualización Diagnóstico del STU de la Conurbación Coquimbo - La Serena",
            ""),
  File = c("Actualiz_diag_STU_Coquimbo_La Serena_Inf_Final.pdf",
           "")
) %>% kbl() %>% kable_classic()

#' ## Definition of a trip 
#' 1. *Trip:* All trips without any restriction
#' 
#' 2. *Collection:* Trips collected in this survey correspond to those made the 
#' during the week (Dia Laboral Normal) and also the weekends (page 11-1 and 
#' page 11-94, cuadro 11.15-9 and 11.15-10).
#' 
...
#' ### Importing datasets
#' In page 11-78 of **File1** there's a data dictionary.
...
# V-Drive folder
#path <- "V:/Studies/MOVED/HealthImpact/Data/Country/Chile/Travel Surveys/Coquimbo_LaSerena/"
# Local folder
path <- "C:/Users/danie/Documents/Daniel_Gil/Consultorias/2020/WorldBank/Data/Chile/Coquimbo_LaSerena/Trips/"
...
#' the same to what is mentioned in page 11-93 (Cuadro 11.15-6) of **File1**.
sum(hh$Factor)
sum(people$Factor)
...
#' Compare this with what is mentioned in page 11-93 (Cuadro 11.15-7) of 
#' **File1**. 
people %>% 
  group_by(IDEstudios) %>% summarise(Total = sum(Factor))

#' Results are not the same, but they are the same when compared to the 
#' database. In table "005-Tabla5" (adding columns in excel). Maybe this is a
#' mistake in the document.
#' 
#' ### Number of hh and people by sector
#' Compare this with what is mentioned in page 11-95 (Cuadro 11.15-11) of
#' **File1**. 
...
# sum(trips2$FactorExpansion, na.rm = T) #OK

trips2 %>% group_by(IDModo) %>%
  summarise(n_trips = sum(FactorExpansion, na.rm = T))
#' Results are the same. Important here is that "other" and "taxi-radio taxi" 
#' (option 5) add to the same number of trips as adding "other" and Taxi basico"
#' in the table (23.155 trips). There's no need to filter any trip here.
#' 
#' # **Preprocessing phase**
#' ## Filtering people from only Coquimbo and La Serena
#' Since the survey was conducted in only comuna Coquimbo and La serena 
...
#'Less than 2% of trips have more than 1 stage
...
#' trips, I will use trip_mode as stage_mode (see trip 102925-6-1 as example).
...
#' In 85 trips the walking duration is the same or larger than the trip
...
report$meta_data[1] <- 448784 # Population in 2017
...
write_csv(report, 'Data/Report/coquimbo_laserena/coquimbo_laserena_trips.csv')
...
write_csv(trips_export, 
          'Data/ITHIM/coquimbo_laserena/trips_coquimbo_laserena.csv')
