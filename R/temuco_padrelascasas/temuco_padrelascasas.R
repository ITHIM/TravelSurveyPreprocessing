#' ---
#' title: "Preprocessing of Temuco's travel dataset"
#' author: "Daniel"
#' output:
#'   html_document:
#'     toc: true
...
#' ## Documentation
#' These files are available in the v-drive in the path "V:/Studies/MOVED/HealthImpact/Data/Country/Chile/Travel Surveys/Temuco_PadreLasCasas/". Locally, this documentation is located in ".../Chile/Temuco_PadreLasCasas/Trips/Reports".
...
data.frame(
  ...
  Description = c("Technical report and final results",
                  ""),
  Title = c("ACTUALIZACIÓN PLAN DE TRANSPORTE TEMUCO Y DESARROLLO DE ANTEPROYECTO",
            ""),
  File = c("Actualizacion_Plan_Transporte_Temuco I Etapa_Inf_Final.pdf",
           "")
) %>% kbl() %>% kable_classic()
...
#' ### Importing datasets
#' In page 12-11 of **File1** there's a data dictionary.
...
# V-Drive folder
#path <- "V:/Studies/MOVED/HealthImpact/Data/Country/Chile/Travel Surveys/Temuco_PadreLasCasas/"
# Local folder
path <- "C:/Users/danie/Documents/Daniel_Gil/Consultorias/2021/Cambridge/Data/Chile/Temuco_PadreLasCasas/Trips/"
...
#' the same to what is mentioned in page 13-6 (Cuadro 13.8) of **File1**.
sum(hh$Factor)
sum(people$Factor)
#' Results are the same.
#' 
#' ### Number of people by education
#' Compare this with what is mentioned in page 13-8 (Cuadro 13.9) of 
#' **File1**. 
people %>% group_by(IDEstudios) %>% summarise(Total = sum(Factor))
#' Results are the same
#' 
#' ### Number of people by macrozone
#' Compare this with what is mentioned in page 13-15 (Cuadro 13.12) of 
#' **File1**. 
...
#' 
#' ### Mode share
#' Compare this with what is mentioned in page 13-49 (Cuadro 13.30) of 
#' **File1**. 
...
#' 
#' # **Preprocessing phase**
#' ## Filtering people from only Temuco and Padre las Casas
#' Since the survey was conducted in comunas Temuco and Padre Las Casas
#' (page 11-4 of **File1**) and there's information in this same coverage about
...
#'Less than 3% of trips have more than 1 stage
...
#' Only in 72 trips the walking duration is the same or larger than the trip
...
report$meta_data[1] <- 358541 # Population in 2017
...
report$meta_data[10] <- "motorcycle" # missing modes 
...
write_csv(report, 
          'Data/Report/temuco_padrelascasas/temuco_padrelascasas_trips.csv')
...
write_csv(trips_export, 
          'Data/ITHIM/temuco_padrelascasas/trips_temuco_padrelascasas.csv')
