#' ---
#' title: "Preprocessing of Valdivia's travel dataset"
#' author: "Daniel"
#' output:
#'   html_document:
#'     toc: true
...
#' ## Documentation
#' These files are available in the v-drive in the path "V:/Studies/MOVED/HealthImpact/Data/Country/Chile/Travel Surveys/Valdivia/". Locally, this documentation is located in ".../Chile/Valdivia/Trips/Reports".
...
data.frame(
  ...
  Description = c("Technical report and final results",
                  ""),
  Title = c("ACTUALIZACIÓN PLAN DE TRANSPORTE VALDIVIA Y DESARROLLO DE ANTEPROYECTO",
            ""),
  File = c("Actualizacion_Plan_Transporte_Valdivia_I Etapa_Inf_Final.pdf",
           "")
) %>% kbl() %>% kable_classic()
...
#' ### Importing datasets
#' In page 12-10 of **File1** there's a data dictionary.
...
# V-Drive folder
#path <- "V:/Studies/MOVED/HealthImpact/Data/Country/Chile/Travel Surveys/Valdivia/"
# Local folder
path <- "C:/Users/danie/Documents/Daniel_Gil/Consultorias/2021/Cambridge/Data/Chile/Valdivia/Trips/"
...
people <- people %>% 
  inner_join(people_age, by = c("IDFolio", "IDPersona" = "IDPERSONA"))
...
#' the same to what is mentioned in page 13-5 (Cuadro 13.8) of **File1**.
sum(hh$Factor)
sum(people$Factor)
#' Results are the same.
#' 
#' ### Number of people by education
#' Compare this with what is mentioned in page 13-7 (Cuadro 13.9) of 
#' **File1**. 
people %>% group_by(IDEstudios) %>% summarise(Total = sum(Factor))
#' Results are the same.
#' 
#' ### Number of hh and people by macrozone
#' Compare this with what is mentioned in page 13-10 (Cuadro 13.12) of 
#' **File1**. 
...
#' 
#' ### Mode share
#' Compare this with what is mentioned in page 13-44 (Cuadro 13.30) of 
#' **File1**. 
sum(trips[trips$valida == 1, "Factor"])
trips %>% filter(valida == 1) %>% 
  group_by(IDModo) %>% summarise(Total = sum(Factor))

#' Results are the same. Bus interprovincial and Buses-taxibus have different
#' frequencies, but since both goes to "bus" category, it doesn't matter.
#' Important here is that is important to filter only
#' valid trips, which can be identified by the column "valida == 1".
#' 
#' # **Preprocessing phase**
#' ## Filtering people from Valdivia only
#' Since the survey was conducted only in comuna Valdivia
#' (page 2-1 of **File1**)
...
#' trips, I will use trip_mode as stage_mode (see trip 3003005-1-1 as example).
...
#' Only in 77 trips the walking duration is the same or larger than the trip
...
report$meta_data[1] <- 166080 # Population in 2017
...
write_csv(report, 'Data/Report/valdivia/valdivia_trips.csv')
...
#' ### Export dataset
write_csv(trips_export, 'Data/ITHIM/valdivia/trips_valdivia.csv')
