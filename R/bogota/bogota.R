...
#' ## Documentation
#' These files are available in the v-drive in the path "V:/Studies/MOVED/HealthImpact/Data/Country/Colombia/Bogota/Travel/trips_2019/". Locally, this documentation is located in "...Colombia/Bogota/Trips/Informes de Indicadores/".
...
#' day of reference, i.e., the day before the survey (page 120 of **File2**).
#' 
...
# People
people <- read_excel(paste0(path, "PersonasEODH2019.xlsx"), guess_max = 100000)
# Vehicles
#vehicles <- read_excel(paste0(path, "VehículosEODH2019.xlsx"))
# Trips
trips <- read_excel(paste0(path, "ViajesEODH2019.xlsx"), guess_max = 100000)

#+ warning=FALSE, message=FALSE, cache=TRUE
# Stages: there's information about walk_to_pt stages
stages <- read_excel(paste0(path, "EtapasEODH2019.xlsx"), guess_max = 100000)


#' ### Number of rows
#' The first thing to do is verify that the number of rows of each dataset is 
#' the same to what is mentioned in page 210 (Tabla 6.1) of **File1**.
nrow(hh)
nrow(people)
nrow(trips)
nrow(stages)
...
#' Compare this with what is mentioned in page 20 (Tabla 2.1) of **File1**. 
#' The SQL code for this output is in page 214 (Paragraph 6.26).
...
#' 
#' ### Average number of people per household by socio-economic strata (using
#'  weights)
...
#' 
#' To get the duration of each trip I used a file that has this already 
#' computed, as well, as the main mode. The computation of the duration is
#' explained in page 202 (paragraphs 6.44 to 6.47). Now, regarding the main 
#' mode, in the documentation says that 
#' **they defined a hierarchy between modes to define ** 
#' **the main mode of each trip**. This hierarchy is in page 126, table 4.4 of
#' **File1**.

# Reading file with trip duration and main mode
duration <- read_excel(paste0(path, "Aux_DuraciónEODH2019.xlsx"))
...
#' **Note 1**: I am using here all trips regardless of walking trips shorter than 
#' 15 minutes. 
#' 
#' **Note 2**: If we choose to work with only walking trips longer than 15 
...
people_bogota <- people_bogota %>% 
  mutate(participant_id_paste = paste(id_hogar, id_persona, sep = "-"))
length(unique(people_bogota$participant_id_paste)) == nrow(people_bogota)
...
#' The variable in stage dataset that have these modes is 
#' *p18_id_medio_transporte*. This is the result:
...
#' 
#' Now with respect to trip purpose, I only had to translate them
#' (p17_Id_motivo_viaje). This is the result:
...
trips_bogota %>% filter(id_hogar == "1117", id_persona == "2", 
                        id_viaje == "1") %>% 
  dplyr::select(id_hogar, id_persona, id_viaje, hora_inicio_viaje,
                p30_camino_minutos, p31_hora_llegada) %>% 
...
stages_bogota %>% filter(id_hogar == "1117", id_persona == "2", 
  ...
  dplyr::select(id_hogar, id_persona, id_viaje, id_etapa, 
                p18_id_medio_transporte, p19_camino_minutos,
                p21_tiempo_arrancar_vehic) %>% 
  kbl() %>% kable_classic()
...
#' level, it's not enough to get the duration for each stage. The information
#' available is about minutes walking before and after taking other mode, but
#' there's no information about the time spent in each stage. 
#' 
...
stages <- stages %>% 
  mutate(stage_id_paste = paste(id_hogar, id_persona, id_viaje, 
                                id_etapa, sep = "-"))
...
#' them to be motorcycle and bus respectively. Thus, I will compute trip_mode
...
stages_bogota_v2 <- stages_bogota %>% 
  ...
  mutate(
    ...
    match(p18_id_medio_transporte, main_mode$Code )],
...
#' modes as it is done in Chilean surveys. I however rename stages dataset
#' so it is consistent with other scripts.
stages_bogota_v3 <- stages_bogota_v2
...
sum(is.na(stages_bogota_v3$p19_camino_minutos))
sum(is.na(stages_bogota_v3$p30_camino_minutos))
stages_bogota_v3_1 <- stages_bogota_v3 %>% filter(n == 1) %>% 
...
#View(stages_bogota_v3_1)
#length(unique(stages_bogota_v3_1$stage_id_paste))
#table(stages_bogota_v3_1$stage_mode, stages_bogota_v3_1$need_adjustment)
#table(stages_bogota_v3_1$need_adjustment)
...
#sum(is.na(stages_bogota_v3$walking_duration))
table(stages_bogota_v3_1$need_adjustment, useNA = "always")
table(stages_bogota_v3_1$need_adjustment, useNA = "always") / nrow(stages_bogota_v3_1)
#View(stages_bogota_v3_1[stages_bogota_v3_1$need_adjustment == 1, ])
#table(stages_bogota_v3_1[stages_bogota_v3_1$need_adjustment == 1, "stage_mode"])
...
#table(stages_bogota_v3_1$trip_mode, stages_bogota_v3_1$stage_mode)
#names(stages_bogota_v3_1)
stages_bogota_v3_1_adjust <- stages_bogota_v3_1 %>% 
...
stages_bogota_v3_1_noadjust <- stages_bogota_v3_1 %>% 
...
# length(unique(stages_bogota_v3_1_noadjust$stage_id_paste)) == 
#   nrow(stages_bogota_v3_1_noadjust) #OK
# sum(is.na(stages_bogota_v3_1_noadjust$stage_duration))
...
# names(stages_bogota_v3_1_adjust)
# names(stages_bogota_v3_1_noadjust)
stages_bogota_v3_1_ready <- stages_bogota_v3_1_adjust %>% 
  bind_rows(stages_bogota_v3_1_noadjust)

# Same number of trips before and after creating walking stages
# length(unique(stages_bogota_v3_1$trip_id_paste)) ==
#   length(unique(stages_bogota_v3_1_ready$trip_id_paste)) ## ok
...
stages_bogota_v3_2 <- stages_bogota_v3 %>% filter(n > 1) %>% 
  mutate(
...
stages_bogota_ready <- stages_bogota_v3_1_ready %>% 
  bind_rows(stages_bogota_v3_2) %>% 
...
length(unique(stages_bogota_v3$trip_id_paste)) 
# Number of trips after processing
length(unique(stages_bogota_v3_1_ready$trip_id_paste)) + 
  length(unique(stages_bogota_v3_2$trip_id_paste)) 
length(unique(stages_bogota_ready$trip_id_paste)) 

# Number of stages before processing
length(unique(stages_bogota_v3$stage_id_paste))
# Number of stages after processing
length(unique(stages_bogota_v3_1_ready$stage_id_paste)) + 
  length(unique(stages_bogota_v3_2$stage_id_paste)) 
...
left_join(stages_bogota_ready, 
          by = c("id_hogar" = "household_id", 
...
write_csv(report, 'Data/Report/bogota/bogota_trips.csv')
...
#' van to car, rickshaw to auto_rickshaw, train to rail.
#' 
#' ### Creating again IDs
